...
class Login extends React.Component {
    ...
    login = () => {
        ...
        var loguser = this.props.users.find(user => user.email === this.state.email)
        if (loguser === undefined) {
            this.setState({ wrongEmail: true })
        } else {
            if (loguser.password === this.state.password) {
                // this.setState({ username: loguser.firstname })
                this.props.userLoggedIn(loguser.firstname)
...
const UserstoreToProps = (store) => { return { users: store.users } }
const userDispatch = (dispatch) => {
    return (bindActionCreators({ userLoggedIn: userBroadCast }, dispatch))
}
export default connect(UserstoreToProps, userDispatch)(Login)